...
function run() {
  ...
  const sharedLoader = read('components/loading-animation/LoadingAnimation.tsx')
  const ringRenderer = read('components/loading-animation/RingRenderer.ts')
  assert.match(sharedLoader, /<canvas/)
  assert.match(sharedLoader, /requestAnimationFrame/)
  assert.match(sharedLoader, /visibilitychange/)
  assert.equal(sharedLoader.includes('<svg'), false)
  assert.equal(sharedLoader.includes('gsap'), false)
  assert.match(ringRenderer, /RING_SEGMENTS = 72/)
  assert.match(ringRenderer, /background: '#000000'/)
