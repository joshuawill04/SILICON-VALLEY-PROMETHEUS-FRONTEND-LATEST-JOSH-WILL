          ...
          return (
            ...
            >
              <span className="relative flex items-start gap-2">
                {active ? (
                  <InlineLoadingAnimation className="mt-0.5" size={12} label={`Running ${step.label}`} />
                ) : (
                  <span
                    className={cn(
                      'mt-1 size-1.5 rounded-full',
                      error ? 'bg-rose-200' : complete ? 'bg-white/72' : 'bg-white/22',
                    )}
                  />
                )}
        ...
        {clip.variants.slice(0, 4).map((variant, index) => (
          ...
          >
            <div className="relative aspect-video overflow-hidden bg-[#05070a]">
              ...
              {loading ? (
                <div className="absolute inset-0 grid place-items-center bg-black/34">
                  <InlineLoadingAnimation size={40} label={`Processing ${variant.title}`} />
                </div>
              ) : null}
                ...
                >
                  ...
                  {posting.captionGenerating ? (
                    <div className="mt-3 flex min-h-12 items-center justify-center">
                      <InlineLoadingAnimation size={40} label={`Generating ${platform.label} caption`} />
                    </div>
            ...
            {latestExport ? (
              <div className="flex min-h-[96px] gap-3 rounded-[20px] border border-white/10 bg-white/[0.035] p-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  >
                    {isDownloading ? (
                      <InlineLoadingAnimation size={16} label="Preparing download" />
                    ) : (
